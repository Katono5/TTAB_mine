...
class ResNetCifar(ResNetBase):
    def __init__(
        ...
        self.in_layer = nn.InstanceNorm2d(num_features=64)
    ...
    def forward_features(self, x):
        ...
        x = self.conv1(x)
        y = self.layer1(x)
        in_out = self.in_layer(y)
        bn_out = nn.BatchNorm2d(64).cuda()(y)
        with open('256.txt', 'a') as f:
            print(torch.dist(in_out, bn_out, p=2), file=f)
        x = self.bn1(x)
        x = self.relu(x)
