...
class FeatureExtractor(nn.Module):
    def __init__(self, model, layer_name):
        ...
        # 将model的子模块（直到layer_name）作为feature部分
        self.features = nn.Sequential(*list(model.children())[:-2])  # 提取到倒数第二层
        # 特定层的名称
    ...
    def one_adapt_step(
        ...
        with timer("forward"):
            with fork_rng_with_seed(random_seed):
                #y_hat, y_feature = model(batch._x)  # adapt
                model.layer1.register_forward_hook(get_activation('layer1'))
                y_hat = model(batch._x)
                layer1_output = activation['layer1']
            ...
            contra_loss =(adaptation_utils.contrastive_loss(layer1_output)) / 10
            print(contra_loss)
            loss += contra_loss / 2
            contra_loss_label = (adaptation_utils.contrastive_loss(layer1_output, labels=torch.argmax(y_hat, dim=1))) / 10
            print(contra_loss_label)
            loss += contra_loss_label / 2
